...
- **No sound has been produced.** Apple's audio drivers start
  ...
  reached the transmit FIFO. The reason is now known: the I²S driver waits for
  two edges on GPIO line 0x86 before it starts DMA, and nothing drove that
  line. A frame clock now drives it. That is not yet tested on a device
  (`docs/audio.md`).
...
| **Audio: modelled, never heard** | The WM8991 codec, both I²S controllers and the PL080 DMA engine are modelled and unit-tested, Apple's `AppleWM8991Audio` starts against them, and the app has a host playback path. On a device **zero** words have reached the transmit FIFO: DMA start waited for two frame-clock edges on GPIO line 0x86 that nothing drove. The I²S model now has a frame clock that drives that line and paces the FIFO. Untested on a device. |
