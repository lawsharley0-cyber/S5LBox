...
- **No sound has been heard.** Since the I²S frame clock was modelled (build
  12b2cba), PCM DMA runs on the device: 1.46 million frames reached the app in
  one report. All of them were zero, because ringtones are AAC and went to the
  unmodelled hardware decoder (AMC). The app now hides that decoder so the
  guest decodes in software; that is not yet tested on a device
  (`docs/audio.md`).
...
| **Audio: modelled, never heard** | The WM8991 codec, both I²S controllers and the PL080 DMA engine are modelled and unit-tested, Apple's `AppleWM8991Audio` starts against them, and the app has a host playback path. With the I²S frame clock modelled, PCM DMA runs on the device, but every sample so far was zero: ringtones are AAC and went to the unmodelled hardware decoder (AMC). The app now hides the AMC so the guest decodes in software. Untested on a device. |
